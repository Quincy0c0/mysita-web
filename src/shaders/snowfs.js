...
in vec2 v_textureCoordinates; // 使用 'in' 替代 'varying'
...
void main() {
    float iTime = (czm_frameNumber / 120.0) * snowSpeed;
    float snow = 0.0;
    ...
    vec4 col = texture(colorTexture, v_textureCoordinates); // 使用 'texture' 替代 'texture2D'

    fragColor = mix(col, snowCol, 0.6); // 使用输出变量
}`;
