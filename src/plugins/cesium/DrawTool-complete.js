 ...
 * @LastEditTime: 2024-09-03 14:48:20
 * 
 */
import * as Cesium from "cesium";
import {getPositionHeight,generateCirclePoints,cartesian3ToDegreesHeight} from './index'
...
class DrawTool {
  ...
  // 存储可以支持的绘制类型，这样用户可以快捷的访问到这些绘制类型
  DrawTypes = {
    Polyline: "Polyline",
    Rect: "Rect",
    Point: "Point",
  ...
  // 用户调用：drawTool.active(drawTool.DrawTypes.Polyline)
  // 激活工具，传入DrawType
  active(drawType) {
    // 如果在我们的绘制工具集合中，存在这个工具
    if (Object.keys(this.DrawTypes).includes(drawType)) {
      this.drawType = drawType;
      // 最终的坐标，每次左键点击的坐标，保存下来
      this.positions = [];
      // 绘制过程中的坐标，专门服务callbackProperty，做动态绘制
      this.curPositions = [];
      //   每次点击有一个标点，需要存储实体
      this.points = [];
      // 注册鼠标事件
